...
export interface ModalCorporateProps {
  onSelect?: (data: DataResult) => void;
  onMutiSelect?: (data: DataResult[]) => void;
  onAdd?: (data: DataResult) => void;
  title?: string;
  type?: FieldType.Choice | FieldType.MutiSelect;
  dataSelected?: DataResult[];
}
const ModalCorporate: FC<ModalCorporateProps> = React.memo((props) => {
  const { onSelect = () => { }, dataSelected, title, type = FieldType.Choice, onMutiSelect = () => { }, onAdd = () => { } } = props;
  ...
  const onFilter = async () => {
    const url = `${serviceUrls.path.corporateDropdown}`;
    setIsLoading(true);
    try {
      const params = {
        name: text,
  ...
  const onChooseItem = (item: DataResult) => {
    if (isMutiSelect) {
      if (listDataSelected.length > 0 && listDataSelected.some((elm) => elm?.label === item?.label)) {
