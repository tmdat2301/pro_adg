...
import { detailsContactMissionRequest, setRefreshingContactMission } from '@redux/actions/detailsActions';
...
const MissionTab = (props: IMissionTab) => {
  ...
  const { objMission, contactId } = useSelector((state: RootState) => state.detailsContactReducer);
  ...
  useEffect(() => {
    if (objMission.load.isRefreshing) {
      dispatch(detailsContactMissionRequest(contactId ?? '', dateFilter, idFilter));
    }
  }, [objMission.load.isRefreshing, contactId]);
  const arrFilter = [
    {
      id: -99,
  ...
  const changeStatus = async (id: number | string) => {
    try {
      const url = `${serviceUrls.path.changeStatusContactTask}${id}?status=true`;
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingContactMission(true))}
            />
          );
        }}
      />
