...
import { detailsContactAppointmentRequest, setRefreshingContactAppointment } from '@redux/actions/detailsActions';
...
const AppointmentTab = (props: IAppointmentTab) => {
  ...
  const { objAppointment, contactId } = useSelector((state: RootState) => state.detailsContactReducer);
  ...
  useEffect(() => {
    if (objAppointment.load.isRefreshing) {
      dispatch(detailsContactAppointmentRequest(contactId ?? '', dateFilter, idFilter));
    }
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingContactAppointment(true))}
            />
          );
        }}
      />
