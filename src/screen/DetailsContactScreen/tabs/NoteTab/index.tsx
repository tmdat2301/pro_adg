...
import { detailsContactNoteRequest, setRefreshingContactNote } from '@redux/actions/detailsActions';
...
const NoteTab = (props: INoteTab) => {
  ...
  const { objNote, contactId } = useSelector((state: RootState) => state.detailsContactReducer);
  ...
  const deleteNote = async (id: string) => {
    try {
      ...
      const url = `${serviceUrls.path.contactDetailsNote}?noteid=${id}`;
  ...
  useEffect(() => {
    if (objNote.load.isRefreshing) {
      dispatch(detailsContactNoteRequest(contactId ?? ''));
    }
  }, [objNote.load.isRefreshing, contactId]);
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingContactNote(true))}
            />
          );
        }}
