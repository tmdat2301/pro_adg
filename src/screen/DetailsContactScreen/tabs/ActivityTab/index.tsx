...
import { detailsContactActivityRequest, setRefreshingContactActivity } from '@redux/actions/detailsActions';
...
const ActivityTab = (props: IActivityTab) => {
  ...
  const { objActivity, contactId } = useSelector((state: RootState) => state.detailsContactReducer);
  ...
  useEffect(() => {
    if (objActivity.load.isRefreshing) {
      if (idFilter === -99) {
        dispatch(detailsContactActivityRequest(contactId ?? '', arrTypeActivityNumber, 100, dateFilter));
      } else {
        const arrFilter = arrTypeActivityNumber.filter((x) => x === idFilter);
        dispatch(detailsContactActivityRequest(contactId ?? '', arrFilter, 100, dateFilter));
      }
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingContactActivity(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingContactActivity(true))}
            />
          );
        }}
      />
