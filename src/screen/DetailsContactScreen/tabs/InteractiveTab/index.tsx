...
import { detailsContactInteractiveRequest, setRefreshingContactInteractive } from '@redux/actions/detailsActions';
...
const InteractiveTab = (props: IInteractiveTab) => {
  ...
  const { objInteractive, contactId } = useSelector((state: RootState) => state.detailsContactReducer);
  ...
  useEffect(() => {
    if (objInteractive.load.isRefreshing) {
      dispatch(detailsContactInteractiveRequest(contactId ?? '', true, dateFilter));
    }
  }, [objInteractive.load.isRefreshing, contactId]);
  const arrFilter = [
    {
      id: -99,
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingContactInteractive(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingContactInteractive(true))}
            />
          );
        }}
      />
