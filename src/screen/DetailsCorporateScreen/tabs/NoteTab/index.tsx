...
import { detailsCorporateNoteRequest, setRefreshingCorporateNote } from '@redux/actions/detailsActions';
...
const NoteTab = (props: INoteTab) => {
  ...
  const { objNote, corporateId } = useSelector((state: RootState) => state.detailsCorporateReducer);
  ...
  const deleteNote = async (id: string) => {
    try {
      ...
      const url = `${serviceUrls.path.corporateDetailsNote}?noteid=${id}`;
  ...
  useEffect(() => {
    if (objNote.load.isRefreshing) {
      dispatch(detailsCorporateNoteRequest(corporateId ?? ''));
    }
  }, [objNote.load.isRefreshing, corporateId]);
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingCorporateNote(true))}
            />
          );
        }}
