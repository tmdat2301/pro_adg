...
import { detailsCorporateAppointmentRequest, setRefreshingCorporateAppointment } from '@redux/actions/detailsActions';
...
const AppointmentTab = (props: IAppointmentTab) => {
  ...
  const { objAppointment, corporateId } = useSelector((state: RootState) => state.detailsCorporateReducer);
  ...
  useEffect(() => {
    if (objAppointment.load.isRefreshing) {
      dispatch(detailsCorporateAppointmentRequest(corporateId ?? '', dateFilter, idFilter));
    }
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingCorporateAppointment(true))}
            />
          );
        }}
      />
