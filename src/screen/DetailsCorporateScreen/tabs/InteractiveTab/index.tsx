...
import { detailsCorporateInteractiveRequest, setRefreshingCorporateInteractive } from '@redux/actions/detailsActions';
...
const InteractiveTab = (props: IInteractiveTab) => {
  ...
  const { objInteractive, corporateId } = useSelector((state: RootState) => state.detailsCorporateReducer);
  ...
  useEffect(() => {
    if (objInteractive.load.isRefreshing) {
      dispatch(detailsCorporateInteractiveRequest(corporateId ?? '', true, dateFilter));
    }
  }, [objInteractive.load.isRefreshing, corporateId]);
  const arrFilter = [
    {
      id: -99,
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingCorporateInteractive(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingCorporateInteractive(true))}
            />
          );
        }}
      />
