...
import { detailsCorporateActivityRequest, setRefreshingCorporateActivity } from '@redux/actions/detailsActions';
...
const ActivityTab = (props: IActivityTab) => {
  ...
  const { objActivity, corporateId } = useSelector((state: RootState) => state.detailsCorporateReducer);
  ...
  useEffect(() => {
    if (objActivity.load.isRefreshing) {
      if (idFilter === -99) {
        dispatch(detailsCorporateActivityRequest(corporateId ?? '', arrTypeActivityNumber, 100, dateFilter));
      } else {
        const arrFilter = arrTypeActivityNumber.filter((x) => x === idFilter);
        dispatch(detailsCorporateActivityRequest(corporateId ?? '', arrFilter, 100, dateFilter));
      }
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingCorporateActivity(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingCorporateActivity(true))}
            />
          );
        }}
      />
