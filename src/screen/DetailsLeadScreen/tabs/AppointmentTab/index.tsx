...
import { detailsLeadAppointmentRequest, setRefreshingLeadAppointment } from '@redux/actions/detailsActions';
...
const AppointmentTab = (props: IAppointmentTab) => {
  ...
  const { objAppointment, leadId } = useSelector((state: RootState) => state.detailsLeadReducer);
  ...
  useEffect(() => {
    if (objAppointment.load.isRefreshing) {
      dispatch(detailsLeadAppointmentRequest(leadId ?? '', dateFilter, idFilter));
    }
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingLeadAppointment(true))}
            />
          );
        }}
      />
