...
import { detailsLeadActivityRequest, setRefreshingLeadActivity } from '@redux/actions/detailsActions';
...
const ActivityTab = (props: IActivityTab) => {
  ...
  const { objActivity, leadId } = useSelector((state: RootState) => state.detailsLeadReducer);
  ...
  useEffect(() => {
    if (objActivity.load.isRefreshing) {
      if (idFilter === -99) {
        dispatch(detailsLeadActivityRequest(leadId ?? '', arrTypeActivityNumber, 100, dateFilter));
      } else {
        const arrFilter = arrTypeActivityNumber.filter((x) => x === idFilter);
        dispatch(detailsLeadActivityRequest(leadId ?? '', arrFilter, 100, dateFilter));
      }
    }
    if (arrTypeActivityNumber.length === 0) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingLeadActivity(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingLeadActivity(true))}
            />
          );
        }}
      />
