...
import { detailsLeadMissionRequest, setRefreshingLeadMission } from '@redux/actions/detailsActions';
...
const MissionTab = (props: IMissionTab) => {
  ...
  const { objMission, leadId } = useSelector((state: RootState) => state.detailsLeadReducer);
  ...
  useEffect(() => {
    if (objMission.load.isRefreshing) {
      dispatch(detailsLeadMissionRequest(leadId ?? '', dateFilter, idFilter));
    }
  }, [objMission.load.isRefreshing, leadId]);
  const arrFilter = [
    {
      id: -99,
  ...
  const changeStatus = async (id: number | string) => {
    try {
      const url = `${serviceUrls.path.changeStatusLeadTask}${id}?status=true`;
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingLeadMission(true))}
            />
          );
        }}
      />
