...
import { detailsLeadFileRequest, setRefreshingLeadFile } from '@redux/actions/detailsActions';
...
const FileTab = (props: FileTab) => {
  ...
  const { objFile, leadId } = useSelector((state: RootState) => state.detailsLeadReducer);
  ...
  }, [objFile.load.isRefreshing, skipCount]);
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingLeadFile(true))}
            />
          );
        }}
        ListFooterComponent={() => {
          if (objFile.arrFile.length >= objFile.totalFile || objFile.load.isRefreshing || objFile.load.isRefreshing) {
            return null;
          }
          return (
