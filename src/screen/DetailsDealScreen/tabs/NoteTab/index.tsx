...
import { detailsDealNoteRequest, setRefreshingDealNote } from '@redux/actions/detailsActions';
...
const NoteTab = (props: INoteTab) => {
  ...
  const { objNote, dealId } = useSelector((state: RootState) => state.detailsDealReducer);
  ...
  const deleteNote = async (id: string) => {
    try {
      ...
      const url = `${serviceUrls.path.dealDetailsNote}?noteid=${id}`;
  ...
  useEffect(() => {
    if (objNote.load.isRefreshing) {
      dispatch(detailsDealNoteRequest(dealId ?? ''));
    }
  }, [objNote.load.isRefreshing, dealId]);
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingDealNote(true))}
            />
          );
        }}
