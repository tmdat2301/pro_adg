...
import { detailsDealMissionRequest, setRefreshingDealMission } from '@redux/actions/detailsActions';
...
const MissionTab = (props: IMissionTab) => {
  ...
  const { objMission, dealId } = useSelector((state: RootState) => state.detailsDealReducer);
  ...
  useEffect(() => {
    if (objMission.load.isRefreshing) {
      dispatch(detailsDealMissionRequest(dealId ?? '', dateFilter, idFilter));
    }
  }, [objMission.load.isRefreshing, dealId]);
  const arrFilter = [
    {
      id: -99,
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingDealMission(true))}
            />
          );
        }}
      />
