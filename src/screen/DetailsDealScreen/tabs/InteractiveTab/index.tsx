...
import { detailsDealInteractiveRequest, setRefreshingDealInteractive } from '@redux/actions/detailsActions';
...
const InteractiveTab = (props: IInteractiveTab) => {
  ...
  const { objInteractive, dealId } = useSelector((state: RootState) => state.detailsDealReducer);
  ...
  useEffect(() => {
    if (objInteractive.load.isRefreshing) {
      dispatch(detailsDealInteractiveRequest(dealId ?? '', true, dateFilter));
    }
  }, [objInteractive.load.isRefreshing, dealId]);
  const arrFilter = [
    {
      id: -99,
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onRefresh={() => dispatch(setRefreshingDealInteractive(true))}
        ...
        ListEmptyComponent={() => {
          return (
            <AppEmptyViewList
              ...
              onReloadData={() => dispatch(setRefreshingDealInteractive(true))}
            />
          );
        }}
      />
